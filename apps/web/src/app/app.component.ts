...
import { UI_TRANSLATIONS, type UiLanguage } from "../features/i18n/ui-translations";
import { TUTORIAL_GUIDES, TUTORIAL_GUIDES_EN, type TutorialGuide, type TutorialStep } from "../features/onboarding/tutorial-guides";
import { CLOUD_PROPERTY_FIELDS, GENERIC_NODE_PROPERTY_FIELDS, NODE_PROPERTY_FIELDS_BY_KIND, CONTAINER_CODE_PROPERTY_KINDS, type NodePropertyField } from "../features/editor/node-property-fields";
import {
  normalizeEdgeStyle,
...
import {
  CODE_LANGUAGE_BADGES,
  CODE_LANGUAGE_OPTIONS,
  detectCodeLanguageFromContent,
  normalizeCodeLanguageValue,
  type CodeLanguage,
  type CodeLanguageBadge,
  type CodeLanguageOption
} from "../features/editor/code-language";
import {
...
export class AppComponent implements AfterViewInit, OnDestroy {
  ...
  getNodeCodeLanguage(node: CanvasNode): CodeLanguage {
    ...
    const fromContent = detectCodeLanguageFromContent(
  ...
  updateSelectedCodeLanguage(language: string): void {
    const value = normalizeCodeLanguageValue(language) ?? "typescript";
  ...
  private updateNodeCodeContent(node: CanvasNode, content: string): void {
    ...
    if (content.trim().length === 0) {
      ...
      nextProperties["codeLanguage"] =
        detectCodeLanguageFromContent(content) ?? this.getPreferredCodeLanguageForKind(node.kind);
    }
      ...
      if (supportsCode) {
        ...
        if (currentContent.length === 0) {
          ...
          if (normalizedLanguage !== currentLanguage) {
            nextProperties["codeLanguage"] = currentLanguage;
            nodeChanged = true;
          }
        }
      }
...
spec:
  ...
  private markNodeMoveInteractionChanged(changedNodeIds: readonly string[]): void {
    this.clearNodeMoveInteractionRenderCaches(changedNodeIds);
    this.scheduleCollaborationViewPublish();
    this.scheduleViewportCheckpointPersist();
    this.requestViewRender();
  }
  ...
  private clearMovedNodeRenderCaches(changedNodeIds: readonly string[]): void {
    ...
    for (const nodeId of changedNodeIds) {
      ...
      for (const descendantId of this.getDescendantIds(nodeId)) {
